...
An insight dataset records eligibility, recurring-availability, latest completed schedule-output and assignment-row revisions. Unchanged sources reuse a cached dataset. Changed sources mark a retained dataset stale with reasons and return it labelled stale; they do not automatically regenerate it. Explicit refresh, cache miss or expiry regenerates from the current request snapshot. Script Cache has a 300-second TTL; malformed, missing, oversize or evicted entries fall back to regeneration and never authorize access.
