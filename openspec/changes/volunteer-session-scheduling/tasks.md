...
- [ ] 10.19 Polish the candidate table: colour "Candidate coverage" green and "Coverage shortfall" red rather than leaving both as plain text, give the note beside the Edit button more breathing room, decide deliberately whether to grey out "Confirm for scheduling" when coverage is short (disabling it also removes the only place the server's refusal message is seen), make an administrator's scope obvious, since the heading names a single centre whenever that centre happens to own every candidate, give each row a centre column so an administrator reading several centres together can tell which centre it belongs to, and reword the "Your candidate intervals and current advisory coverage" caption, which reads oddly under an administrator's cross-centre view
- [ ] 10.20 Reconsider what the candidate table is for once an interval is confirmed: a confirmed candidate keeps its row among the proposals even though the decision is made and its occurrence now lives in the schedule, and the user asked for a calendar-style view of sessions with monthly and weekly options instead of a flat list tied to the workbook's raw fields
