...
- [ ] 5.7 Browser-verify own-data isolation, a recurring update, a one-off Thursday absence, successful backup promotion, and no-backup alert behavior. Automated acceptance coverage added on 2026-09-20 now proves caller-only assignment, exception, and session projection; caller-scoped recurring replacement with normalization, staleness, administrator notification, and preservation of another volunteer's rows; date-scoped Thursday absence precedence without changing recurring coverage; successful promotion; and understaffed fallback when no backup remains eligible. The focused availability contracts and full suite pass. An authenticated administrator session is now available in the in-app browser, but this acceptance flow still requires a volunteer account and production mutations; it was not performed without an approved snapshot/write-gate procedure
