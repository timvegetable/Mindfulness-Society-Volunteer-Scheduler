...
- [ ] 10.12 Keep every other session's backups when a promotion runs: BackupPromotionService lists only the cancelled session's backups and then replaces the whole Backups tab with that subset, so a single cancellation destroys the backups for the entire published schedule. Verified against the audit trail, which records the tab-wide replace shrinking 55 -> 1 rows on the Monday cancellation and 54 -> 0 on the Friday one, with the promotion payload naming only the cancelled session. Merge the session's rebuilt rows back into the full set, and add a regression test with two sessions' backups that cancels one
